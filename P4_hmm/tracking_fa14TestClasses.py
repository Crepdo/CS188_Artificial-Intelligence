    ...
    def final(self, gameState):
        ...
        for index,inf in enumerate(self.inferenceModules):
            if self.observe:
                inf.observe(gameState)
            self.ghostBeliefs[index] = inf.getBeliefDistribution()
            # inf_dist.append(self.ghostBeliefs[index])
            if self.numMoves >= self.burnIn:
                self.distCompare(self.ghostBeliefs[index], dists[index])
        # dict_inf = ''
        # dict_ref = ''
        # for inf, ref in zip(inf_dist, dists):
        #     # for index_ref, ref in enumerate(dists):
        #     strs = ''
        #     strs_ref = ''
        #     for key in ref.keys():
        #         if key in inf.keys():
        #             strs += f'{key}: {inf[key]}, '
        #         strs_ref += f'{key}: {ref[key]}, '
        #     dict_inf += '{' + strs + '}, '
        #     dict_ref += '{' + strs_ref + '}, '
        #
        # dict_inf = moveNum + ' ' + action + ' [' + dict_inf + ']'
        # dict_ref = moveNum + ' ' + action + ' [' + dict_ref + ']'
        #
        # if dict_ref != dict_inf:
        #     with open('origin.txt', 'a+') as f:
        #         f.write(dict_ref + '\n')
        #     with open('change.txt', 'a+') as c:
        #         c.write(dict_inf + '\n')
        self.display.updateDistributions(self.ghostBeliefs)
        random.seed(self.seed + self.numMoves)
