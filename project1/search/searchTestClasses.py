...
class ApproximateAgentGrade(testClasses.TestCase):

    def __init__(self, question, testDict):
        super(ApproximateAgentGrade, self).__init__(question, testDict)
        self.layoutName = testDict['layoutName']
        ...
        try:
            ...
            try:
                ...
                if passed >= len(self.thresholds):
                    ...
                    grades.addMessage('FAIL: %s' % self.path)
                grades.addMessage('\tApproximate search agent run-time: %1.2f' % extra_time)
                grades.addMessage('\tApproximate search agent total moves %d' % len(moves))
                ...
                grades.addMessage('FAIL: %s' % self.path)              
                grades.addMessage('\tApproximate search agent code is too slow')
                return False
        except Exception as inst:
            grades.addMessage('FAIL: %s' % self.path)              
            grades.addMessage('\tApproximate search agent threw an exception: %s.\n%s' % (str(inst), traceback.format_exc()))
        except:
            grades.addMessage('FAIL: %s' % self.path)              
            grades.addMessage('\tApproximate search agent threw a string exception')
            return False
